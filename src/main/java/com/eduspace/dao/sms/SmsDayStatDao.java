...
public class SmsDayStatDao extends Dao<SmsDayStat>implements SmsDayStatDaoI {

	@Override
	public Integer get(String productId, String date) {
		SQL sql = new SQL();
		
		sql.SELECT().PUT_KEY("DAY_Id").FROM().TABLE(SmsDayStat.class).WHERE().LIKE("date", date).AND().EQ("productId", productId);
	...
	public BigDecimal getNumber(String productId, String startDate, String endDate, String messageState) {
		...
		sql = sql.AS("number").FROM().TABLE(SmsDayStat.class).WHERE().EQ("productId", productId).AND().DATE("date", startDate, endDate);
		Map<String, Object> map = jdbc.executeQueryM(sql.toString());
